...
public class ServerThread implements Runnable {
	...
	public void receiveRemoveShape(JSONObject receiveDrawJson) {
		...
		try {
			if (receiveDrawJson != null) {
				...
				log.recordLog("Client "+clientNumber+" delete a "+shapeName);
			    ...
			    for(ShapeList i:window.getDrawMethod().listShape) {
					...
					if(i.shapeInitialX==shapeInitialX&&
				...
				try {
					//System.out.print("removed object: "+selectedShape);
					window.getDrawMethod().listShape.remove(selectedShape);
				}
				catch(Exception e) {
		...
		try {
			...
			if(line != null) {
				...
				case "updateDraw":
					...
					break;
				case "removeShape":
					receiveRemoveShape(receiveTextJson);
					break;
				default:break;
				}
			}
	...
	public void sendDraw(ShapeList shaplist) {
		...
		try {
			...
			sendTextJson.put("shapeText",shaplist.shapeText);
