...
public class DrawMethod extends MouseAdapter implements ActionListener,Serializable{
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if (shape.equals("line")) {
        	...
        	//window.textArea.setText("gogo");
        	window.getWhiteBoardPanel().add(window.textArea);
        	window.textArea.addKeyListener(listener);
			//ShapeList shaplist1 = new ShapeList(initialX, initialY, finalX, finalY, shape, color, width,window.textArea.getText() ,frameWidth,fontWidth,style);
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        if(shape.equals("chooseshape")) {
        	if(selectedShape!=null) {
        		selectedShape.changeSize(finalX, finalY);
        		...
        		if(window.getIsServer()) {
        			...
        			window.getClient().sendUpdateDraw(selectedShape, 0, 0);
        		}
        	}}
...
public void drawAL(int sx, int sy, int ex, int ey, Graphics2D g2)
	{
		...
		double H = 10; // 缁狀厼銇旀妯哄
		double L = 4; // 鎼存洝绔熼惃鍕閸楋拷
		int x3 = 0;
		int y3 = 0;
		int x4 = 0;
		int y4 = 0;
		double awrad = Math.atan(L / H); // 缁狀厼銇旂憴鎺戝
		double arraow_len = Math.sqrt(L * L + H * H); // 缁狀厼銇旈惃鍕毐鎼达拷
		double[] arrXY_1 = rotateVec(ex - sx, ey - sy, awrad, true, arraow_len);
		double[] arrXY_2 = rotateVec(ex - sx, ey - sy, -awrad, true, arraow_len);
		double x_3 = ex - arrXY_1[0]; // (x3,y3)閺勵垳顑囨稉锟界粩顖滃仯
		double y_3 = ey - arrXY_1[1];
		double x_4 = ex - arrXY_2[0]; // (x4,y4)閺勵垳顑囨禍宀�顏悙锟�
	...
	public double[] rotateVec(int px, int py, double ang,
		...
		// 閻垽鍣洪弮瀣祮閸戣姤鏆熼敍灞藉棘閺佹澘鎯堟稊澶婂瀻閸掝偅妲竫閸掑棝鍣洪妴浜傞崚鍡涘櫤閵嗕焦妫嗘潪顒冾潡閵嗕焦妲搁崥锔芥暭閸欐﹢鏆辨惔锔猴拷浣规煀闂�鍨
    ...
    public void removeOneShape(ShapeList shape) {
    	//ShapeList selectedShape=null;
    	listShape.remove(shape);
    	//System.out.println("removed one case");
    }
